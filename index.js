...
// 2. Veritabanı Modeli (Merkezi Koruma Altında Şifre)
const CarSchema = new mongoose.Schema({
  ...
  supplierPassword: { type: String, default: "flexi2026" }, // Senin korumandaki kurumsal şifre
...
<html lang="tr" class="h-full" style="background-color: #ded9d0;">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    body { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #ded9d0; color: #1c1917; }
    [x-cloak] { display: none !important; }
    .gold-border { border-color: rgba(217, 119, 6, 0.35); }
    .gold-badge { background-color: rgba(251, 191, 36, 0.2); color: #b45309; border: 1px solid rgba(217, 119, 6, 0.35); }
...
<body class="h-full flex flex-col justify-between" x-data="adminApp()">
  ...
  <footer class="w-full py-6 text-center text-xs text-stone-500 border-t border-stone-300 bg-stone-200/50 backdrop-blur-sm">
...
// 5. TEDARİKÇİ PORTALI (Senin Korumandaki Şifreli Giriş Sürümü)
  ...
  <div>
    <header class="bg-white border-b gold-border sticky top-0 z-40 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2" x-show="isLoggedIn">
          ...
          <button @click="activeTab = 'add'" :class="activeTab === 'add' ? 'bg-amber-600 text-white shadow-lg ring-2 ring-amber-500' : 'gold-btn shadow-lg shadow-amber-600/20 hover:scale-105'" class="px-4 py-2.5 rounded-xl font-black text-xs transition-all flex items-center border gold-border">
            <span class="w-5 h-5 rounded-full bg-white text-amber-800 flex items-center justify-center mr-2 text-xs font-black shadow-inner"><i class="fa-solid fa-plus"></i></span> Yeni Araç Ekle
          </button>
    ...
    <main class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col justify-center">
      ...
      <div x-show="!isLoggedIn" class="max-w-md mx-auto bg-white border gold-border rounded-3xl p-8 shadow-sm text-center">
        ...
        <h2 class="text-2xl font-black text-stone-900 mb-2">Tedarikçi Güvenli Giriş</h2>
        <p class="text-xs font-semibold text-stone-600 mb-6">Firma adınızı ve size tanımlanan şifreyi girerek panelinize erişin.</p>
        ...
        <form @submit.prevent="loginSupplier()" class="space-y-4">
          ...
          <button type="submit" class="w-full gold-btn font-extrabold py-3 rounded-xl shadow transition-all text-sm">Güvenli Giriş Yap <i class="fa-solid fa-arrow-right ml-2"></i></button>
        </form>
      </div>
      ...
      <div x-show="isLoggedIn" x-cloak class="w-full space-y-6">
        ...
        <div x-show="activeTab === 'cars'" x-transition>
          <div class="flex justify-between items-center mb-6">
            ...
            <button @click="activeTab = 'add'" class="gold-btn font-bold px-4 py-2 rounded-xl text-xs transition-all shadow flex items-center"><span class="w-4 h-4 rounded-full bg-white text-amber-800 flex items-center justify-center mr-1.5 text-[10px]"><i class="fa-solid fa-plus"></i></span> Yeni Araç Ekle</button>
          </div>
          ...
          <form @submit.prevent="submitCar" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-4">
                ...
                <div>
                  <label class="block text-[10px] font-bold text-stone-500 uppercase tracking-wider mb-1">Paneme Giriş Şifreniz</label>
    ...
    document.addEventListener('alpine:init', () => {
      Alpine.data('supplierPortal', () => ({
        ...
        loginSupplier() {
          ...
          // Senin belirlediğin şifre kontrolü (Eğer araçta şifre kayıtlı değilse varsayılan 'flexi2026' kabul edilir)
          const validPass = supplierCars[0].supplierPassword || 'flexi2026';
          if (enteredPass !== validPass) {
            this.loginError = 'Hatalı şifre! Lütfen yöneticinizle (FlexiDrive HQ) iletişime geçin.';
            return;
          }
...
app.listen(PORT, () => {
  console.log(`FlexiDrive şifre korumalı VIP sunucusu http://localhost:${PORT} adresinde aktif!`);
});
