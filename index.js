...
// 4. KURUMSAL ADMIN PANELİ (Aktif Araç Simgeli Tedarikçi Raporu)
app.get('/', (req, res) => {
  res.send(`<!DOCTYPE html>
<html lang="tr" class="h-full" style="background-color: #1a1816;">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    body { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #1a1816; color: #f5f5f4; }
    ...
    ::-webkit-scrollbar-track { background: #141210; border-radius: 4px; }
...
<body class="h-full flex flex-col justify-between" x-data="adminApp()">
  ...
  <div>
    <header class="bg-[#141210] border-b gold-border sticky top-0 z-40 shadow-xl">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        
        <!-- SOL ÜST LOGO VE AÇILIR MENÜ -->
        <div class="relative" x-data="{ menuOpen: false }">
          ...
          <div x-show="menuOpen" @click.outside="menuOpen = false" x-cloak class="absolute left-0 mt-3 w-56 bg-[#141210] border gold-border rounded-2xl shadow-2xl py-2 z-50 text-xs font-bold text-stone-200">
        ...
        <div class="flex items-center space-x-3">
          ...
          <div class="relative ml-2" x-data="{ langOpen: false }">
            <button @click="langOpen = !langOpen" class="bg-[#1a1816] border gold-border text-amber-400 px-3 py-2 rounded-xl font-black text-xs flex items-center shadow">
              <i class="fa-solid fa-globe mr-1.5"></i> <span x-text="currentLang.toUpperCase()"></span>
            </button>
            <div x-show="langOpen" @click.outside="langOpen = false" x-cloak class="absolute right-0 mt-2 w-36 bg-[#141210] border gold-border rounded-xl shadow-2xl py-1 z-50 text-xs font-bold text-stone-200">
    ...
    <main class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div x-show="activeTab === 'admin'" x-transition>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div class="bg-[#141210] border gold-border p-6 rounded-3xl shadow-lg flex justify-between items-center overflow-hidden">
            <div>
          ...
          <template x-for="car in cars" :key="car._id">
            <div class="car-card-bg border gold-border rounded-3xl p-6 flex flex-col justify-between shadow-xl hover:border-amber-400 transition-all">
              <div>
                ...
                <div class="bg-[#141210]/80 backdrop-blur-sm p-3 rounded-2xl my-4 text-xs space-y-1.5 border border-stone-800 shadow-inner text-stone-200">
      ...
      <!-- TEDARİKÇİ AĞI SEKMESİ (Aktif Araç Simgeli ve Sayılı Yapı) -->
      <div x-show="activeTab === 'partners'" x-cloak x-transition>
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-xs font-semibold text-stone-400 mt-1" x-text="t('supplierReportSub')">Ülkelere göre gruplanmış tedarikçi firmalarınız ve anlık aktif araç durumları</p>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <template x-for="group in groupedSuppliersByCountry" :key="group.country">
            <div class="bg-[#141210] border gold-border rounded-3xl p-6 shadow-xl flex flex-col justify-between">
              <div>
                ...
                <div class="space-y-3 max-h-96 overflow-y-auto pr-2">
                  <template x-for="supplier in group.suppliers" :key="supplier.name">
                    <div class="bg-[#1a1816] border border-stone-800 rounded-2xl p-4 flex justify-between items-center text-stone-200">
                      <div>
                      ...
                      <div class="flex items-center space-x-2">
                        <!-- Ufacak Yeşil Aktif/Müsait Araç Simgesi ve Sayısı -->
                        <div class="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-2.5 py-1 rounded-lg text-[10px] font-black flex items-center shadow-inner" title="Aktif / Müsait Araç Sayısı">
                          <i class="fa-solid fa-circle-check mr-1.5 text-[9px]"></i> <span x-text="supplier.activeCars"></span> Aktif
                        </div>
      ...
      <div x-show="activeTab === 'integrations'" x-cloak x-transition>
        <div class="bg-[#141210] border gold-border rounded-3xl p-8 shadow-xl">
          ...
          <div class="bg-[#1a1816] p-6 rounded-2xl border border-stone-800 space-y-2">
            ...
            <div class="flex space-x-2">
              <input type="text" readonly :value="windowOrigin + '/api/feed/global-inventory'" class="w-full bg-[#141210] border border-stone-700 rounded-xl px-4 py-3 text-xs text-amber-400 font-mono font-bold focus:outline-none">
  ...
  <footer class="w-full py-6 text-center text-xs text-stone-500 border-t border-stone-900 bg-[#141210]/60 backdrop-blur-sm" x-text="t('footer')">
...
<body class="h-full flex flex-col justify-between" x-data="supplierPortal()">
  ...
  <div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2" x-show="isLoggedIn">
          ...
          <button @click="activeTab = 'add'" :class="activeTab === 'add' ? 'bg-amber-500 text-stone-950 shadow-lg ring-2 ring-amber-400' : 'gold-btn shadow-lg shadow-amber-600/20 hover:scale-105'" class="px-4 py-2.5 rounded-xl font-black text-xs transition-all flex items-center border gold-border">
            <span class="w-5 h-5 rounded-full bg-[#141210] text-amber-400 flex items-center justify-center mr-2 text-xs font-black shadow-inner"><i class="fa-solid fa-plus"></i></span> <span x-text="t('addCar')">Yeni Araç Ekle</span>
          </button>
    ...
    <main class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col justify-center">
      ...
      <div x-show="!isLoggedIn" class="max-w-md mx-auto bg-[#141210] border gold-border rounded-3xl p-8 shadow-2xl text-center">
        ...
        <form @submit.prevent="loginSupplier()" class="space-y-4">
          <input type="text" x-model="inputCompanyName" required placeholder="Firma Adınız (Örn: budvarent)" class="w-full bg-[#1a1816] border border-stone-700 rounded-xl px-4 py-3 text-white text-sm text-center font-bold focus:outline-none focus:border-amber-400">
          <input type="password" x-model="inputPassword" required placeholder="Giriş Şifreniz" class="w-full bg-[#1a1816] border border-stone-700 rounded-xl px-4 py-3 text-white text-sm text-center font-bold focus:outline-none focus:border-amber-400">
      ...
      <div x-show="isLoggedIn" x-cloak class="w-full space-y-6">
        ...
        <div @click="activeTab = 'cars'" class="bg-[#141210] border gold-border rounded-3xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-center cursor-pointer hover:border-amber-400 transition-all">
          ...
          <div class="flex space-x-4 bg-[#1a1816] p-3 rounded-2xl border border-stone-800 text-xs text-center text-stone-200">
        ...
        <div x-show="activeTab === 'cars'" x-transition>
          <div class="flex justify-between items-center mb-6">
            ...
            <button @click="activeTab = 'add'" class="gold-btn font-bold px-4 py-2 rounded-xl text-xs transition-all shadow flex items-center"><span class="w-4 h-4 rounded-full bg-[#141210] text-amber-400 flex items-center justify-center mr-1.5 text-[10px]"><i class="fa-solid fa-plus"></i></span> <span x-text="t('addCar')">Yeni Araç Ekle</span></button>
          </div>
            ...
            <template x-for="car in myCars" :key="car._id">
              <div class="car-card-bg border gold-border rounded-3xl p-6 shadow-xl hover:shadow-2xl transition-all flex flex-col justify-between">
                <div>
                  ...
                  <div class="bg-[#1a1816]/80 backdrop-blur-sm p-3 rounded-2xl my-4 text-xs space-y-1.5 border border-stone-800 shadow-inner text-stone-200">
        ...
        <div x-show="activeTab === 'wallet'" x-cloak x-transition>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="bg-[#141210] border gold-border rounded-3xl p-6 shadow-xl">
        ...
        <div x-show="activeTab === 'loyalty'" x-cloak x-transition>
          ...
          <div class="bg-[#141210] border gold-border rounded-3xl p-8 shadow-2xl relative overflow-hidden text-stone-200">
            ...
            <div class="bg-[#1a1816] border border-stone-800 rounded-2xl p-6 text-center space-y-4">
        ...
        <div x-show="activeTab === 'stats'" x-cloak x-transition>
          ...
          <div class="bg-[#141210] border gold-border rounded-3xl p-6 space-y-4 shadow-xl text-stone-200">
        ...
        <div x-show="activeTab === 'add'" x-cloak x-transition class="bg-[#141210] border gold-border rounded-3xl p-8 shadow-2xl relative overflow-hidden text-stone-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-4">
                <div>
                  ...
                  <select x-model="form.country" @change="updateCountryData(form.country)" required class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-4 py-3 text-white text-sm font-bold max-h-48 overflow-y-auto">
                ...
                <div>
                  ...
                  <select x-model="form.airports" required :disabled="!form.country" class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-4 py-3 text-white text-sm font-bold disabled:opacity-40 max-h-48 overflow-y-auto">
                ...
                <div>
                  ...
                  <input type="text" x-model="form.supplierPassword" required placeholder="Örn: flexi2026" class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-4 py-3 text-white text-sm font-bold">
                </div>
                <div>
                  ...
                  <div class="flex space-x-2">
                    ...
                    <input type="tel" x-model="form.phoneOnly" required placeholder="5XX XXX XX XX" class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-4 py-3 text-white text-sm font-mono">
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <div class="flex space-x-3">
                  <div class="w-1/2">
                    ...
                    <select x-model="form.brand" @change="form.model = ''; availableModels = carData[form.brand] || []" required class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-3 py-3 text-white text-sm font-bold max-h-48 overflow-y-auto">
                  ...
                  <div class="w-1/2">
                    ...
                    <select x-model="form.model" required :disabled="!form.brand" class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-3 py-3 text-white text-sm font-bold disabled:opacity-40 max-h-48 overflow-y-auto">
                ...
                <div class="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <input type="number" x-model="form.year" required min="2000" max="2027" class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-3 py-3 text-white text-sm font-bold">
                  </div>
                  <div>
                    <label class="block text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1" x-text="t('category')">Sınıf</label>
                    <select x-model="form.category" required class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-2 py-3 text-white text-sm font-bold">
                  ...
                  <div>
                    ...
                    <select x-model="form.fuelType" required class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-2 py-3 text-white text-sm font-bold">
                ...
                <div class="flex space-x-3">
                  <div class="w-1/3">
                    ...
                    <input type="number" x-model="form.luggageCapacity" required min="0" max="10" placeholder="Adet" class="w-full bg-[#1a1816] border border-stone-800 rounded-xl px-3 py-3 text-white text-sm font-bold">
                  </div>
                  <div class="w-2/3">
                    ...
                    <div class="relative">
                      ...
                      <input type="number" x-model="form.supplierPrice" required min="1" max="400" placeholder="Max 400" class="w-full bg-[#1a1816] border-2 border-stone-700 rounded-xl pl-8 pr-3 py-3 text-white text-sm font-black focus:border-amber-400">
                    </div>
                  </div>
                </div>
              </div>
...
app.listen(PORT, () => {
  console.log(`FlexiDrive aktif araç simgeli VIP sunucusu http://localhost:${PORT} adresinde aktif!`);
});
