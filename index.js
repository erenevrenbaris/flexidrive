...
// 4. ADMIN HQ (Red & Gold Edition + Tam Donanımlı Operasyon Paneli)
app.get('/', (req, res) => {
...
<body class="h-full flex flex-col justify-between" x-data="adminApp()">
  ...
  <div x-show="!isAdminLoggedIn" class="fixed inset-0 z-50 flex items-center justify-center bg-[#0d0d0d] p-4">
    <div class="max-w-md w-full glass-card rounded-3xl p-8 shadow-2xl text-center border gold-border">
      ...
      <form @submit.prevent="loginAdmin()" class="space-y-4">
        ...
        <button type="submit" class="w-full gold-btn font-extrabold py-3.5 rounded-xl shadow-lg transition-all text-sm" x-text="t('adminLoginBtn')">Güvenli Giriş Yap</button>
      </form>
    </div>
  </div>
      ...
      <div @click.outside="selectedCar = null" class="max-w-lg w-full glass-card rounded-3xl p-8 shadow-2xl border gold-border relative text-stone-200">
        ...
        <div class="space-y-4 text-xs">
          <div class="bg-[#1a1616] p-4 rounded-2xl border border-stone-800 space-y-2">
            <div class="flex justify-between items-center"><span class="text-stone-400" x-text="t('modalLocation')">Ülke / Konum:</span><strong class="text-yellow-400 text-sm" x-text="selectedCar ? selectedCar.country + ' - ' + selectedCar.airports : ''"></strong></div>
            <div class="flex justify-between items-center"><span class="text-stone-400" x-text="t('modalSupplier')">Tedarikçi Firma:</span><strong class="text-white" x-text="selectedCar ? selectedCar.supplierName : ''"></strong></div>
            <div class="flex justify-between items-center"><span class="text-stone-400" x-text="t('modalContact')">İletişim (Telefon):</span><strong class="text-emerald-400 font-mono" x-text="selectedCar ? selectedCar.supplierContact : ''"></strong></div>
          </div>

          <div class="grid grid-cols-3 gap-3">
            <div class="bg-[#1a1616] p-3 rounded-xl border border-stone-800 text-center">
              <span class="text-[10px] text-stone-400 block uppercase font-bold" x-text="t('year')">Yıl</span>
              <span class="text-base font-black text-white" x-text="selectedCar ? selectedCar.year : ''"></span>
            </div>
            <div class="bg-[#1a1616] p-3 rounded-xl border border-stone-800 text-center">
              <span class="text-[10px] text-stone-400 block uppercase font-bold" x-text="t('fuel')">Yakıt</span>
              <span class="text-base font-black text-yellow-400" x-text="selectedCar ? selectedCar.fuelType : ''"></span>
            </div>
            <div class="bg-[#1a1616] p-3 rounded-xl border border-stone-800 text-center">
              <span class="text-[10px] text-stone-400 block uppercase font-bold" x-text="t('luggage')">Bavul</span>
          ...
          <div class="bg-[#1a1616] p-4 rounded-2xl border border-stone-800 flex justify-between items-center">
            <div>
              <span class="text-[10px] text-stone-400 block uppercase font-bold" x-text="t('netSale')">Finansal Dağılım</span>
            ...
            <span :class="selectedCar && selectedCar.available ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30' : 'text-rose-400 bg-rose-500/10 border-rose-500/30'" class="px-3 py-1.5 rounded-lg text-xs font-black border" x-text="selectedCar && selectedCar.available ? t('available') : t('rented')"></span>
          </div>
          ...
          <a :href="'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent(selectedCar ? selectedCar.airports + ' ' + selectedCar.country : '')" target="_blank" class="w-full gold-btn font-extrabold py-3.5 rounded-xl shadow-lg transition-all flex items-center justify-center text-xs">
            <i class="fa-solid fa-map-location-dot mr-2 text-sm"></i> <span x-text="t('mapBtn')">Haritada Konumu Göster (Google Maps)</span>
          </a>
        </div>
      </div>
    </div>

    <!-- TEDARİKÇİ VERGİ DOSYASI MODALI (Ad, Soyad, E-posta, VKN ve Tüm Bilgiler Eksiksiz) -->
    <div x-show="selectedTaxSupplier" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
      <div @click.outside="selectedTaxSupplier = null" class="max-w-lg w-full glass-card rounded-3xl p-8 shadow-2xl border gold-border relative text-stone-200">
        ...
        <div class="flex items-center space-x-3 mb-6 border-b border-stone-800 pb-4">
          ...
          <div>
            <span class="text-[9px] font-black px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 uppercase" x-text="t('taxBadge')">Vergi & Mali Dosya</span>
        ...
        <div class="space-y-4 text-xs">
          <div class="bg-[#1a1616] p-5 rounded-2xl border border-stone-800 space-y-3">
            <div class="flex justify-between items-center pb-2 border-b border-stone-800">
              <span class="text-stone-400 font-medium" x-text="t('fullNameLabel')">Yetkili Adı Soyadı:</span>
              <strong class="text-white font-bold" x-text="selectedTaxSupplier ? (selectedTaxSupplier.fullName || 'Eren Evren Barış') : ''"></strong>
            </div>
            <div class="flex justify-between items-center pb-2 border-b border-stone-800">
              <span class="text-stone-400 font-medium" x-text="t('emailLabel')">E-Posta Adresi:</span>
              <strong class="text-yellow-400 font-mono" x-text="selectedTaxSupplier ? (selectedTaxSupplier.email || 'eren@coca-cola.com') : ''"></strong>
            </div>
            <div class="flex justify-between items-center pb-2 border-b border-stone-800">
              <span class="text-stone-400 font-medium" x-text="t('taxNoLabel')">Vergi Kimlik Numarası (VKN):</span>
            ...
            <div class="flex justify-between items-center pb-2 border-b border-stone-800">
              <span class="text-stone-400 font-medium" x-text="t('taxOfficeLabel')">Vergi Dairesi:</span>
              <strong class="text-white" x-text="selectedTaxSupplier ? (selectedTaxSupplier.taxOffice || 'Merkez Kurumsal VD') : ''"></strong>
            </div>
            <div class="flex justify-between items-center pb-2 border-b border-stone-800">
              <span class="text-stone-400 font-medium" x-text="t('taxAddressLabel')">Kayıtlı Ticari Adres:</span>
            ...
            <div class="flex justify-between items-center">
              <span class="text-stone-400 font-medium" x-text="t('modalContact')">İletişim Hattı:</span>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-[#1a1616] p-4 rounded-xl border border-stone-800 text-center">
              <span class="text-[10px] text-stone-400 block uppercase font-bold mb-1" x-text="t('totalCars')">Toplam Araç Hacmi</span>
              <span class="text-xl font-black text-white" x-text="selectedTaxSupplier ? selectedTaxSupplier.carCount + ' Adet' : '0'"></span>
            </div>
            <div class="bg-[#1a1616] p-4 rounded-xl border border-stone-800 text-center">
              <span class="text-[10px] text-stone-400 block uppercase font-bold mb-1" x-text="t('availableCars')">Aktif Müsait Araç</span>
              <span class="text-xl font-black text-emerald-400" x-text="selectedTaxSupplier ? selectedTaxSupplier.activeCars + ' Adet' : '0'"></span>
            </div>
          </div>

          <div class="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-2xl text-center text-emerald-400 font-bold" x-text="t('taxStatusText')">
  ...
  <script>
    const TRANSLATIONS = {
      tr: { 
        adminLoginTitle: 'FlexiDrive Admin HQ', adminLoginSub: 'Yönetici paneline erişmek için admin şifrenizi girin.', adminLoginBtn: 'Güvenli Giriş Yap',
        quickMenu: 'Hızlı Menü', fleet: 'Filo Operasyonları', suppliers: 'Tedarikçi Ağı', feed: 'Meta-Search Feed', supplierPortal: 'Tedarikçi Portalı', 
        supplierBadge: 'Tedarikçi', totalFleet: 'Toplam Filo', activeAvailable: 'Aktif / Müsait', dailyRevenue: 'Günlük Potansiyel Ciro', 
        available: 'MÜSAİT', rented: 'KİRADA', netSale: 'Net / Satış:', published: 'Yayınlanma:', changeStatus: 'Durum Değiştir', removeCar: 'Aracı Kaldır', 
        supplierReportTitle: 'Ülke Bazlı Tedarikçi Hacim Raporu', supplierReportSub: 'Ülkelere göre gruplanmış tedarikçi firmalarınız', 
        metaTitle: 'Meta-Search Entegrasyon Merkezi', metaSub: 'Skyscanner ve Kayak gibi platformların envanterinizi çekeceği açık API adresi.', 
        feedAddress: 'Resmi JSON Feed Bağlantı Adresi', copy: 'Kopyala', footer: 'Tüm Hakları Saklıdır © 2026 FlexiDrive Global OS.',
        ...
        dailyNetEarn: 'Günlük Net Kazanç (Max 400 €)', saveAndPublish: 'Aracı Sisteme Kaydet ve Listeme Ekle',
        modalLocation: 'Ülke / Konum:', modalSupplier: 'Tedarikçi Firma:', modalContact: 'İletişim (Telefon):', mapBtn: 'Haritada Konumu Göster (Google Maps)',
        taxBadge: 'Vergi & Mali Dosya', fullNameLabel: 'Yetkili Adı Soyadı:', taxNoLabel: 'Vergi Kimlik Numarası (VKN):', taxOfficeLabel: 'Vergi Dairesi:', taxAddressLabel: 'Kayıtlı Ticari Adres:', taxStatusText: 'Mali Mükellefiyet Durumu: Aktif ve Uyumlu'
      },
      en: { 
        adminLoginTitle: 'FlexiDrive Admin HQ', adminLoginSub: 'Enter your admin password to access HQ.', adminLoginBtn: 'Secure Login',
        quickMenu: 'Quick Menu', fleet: 'Fleet Operations', suppliers: 'Supplier Network', feed: 'Meta-Search Feed', supplierPortal: 'Supplier Portal', 
        supplierBadge: 'Supplier', totalFleet: 'Total Fleet', activeAvailable: 'Active / Available', dailyRevenue: 'Daily Potential Revenue', 
        available: 'AVAILABLE', rented: 'RENTED', netSale: 'Net / Sale:', published: 'Published:', changeStatus: 'Toggle Status', removeCar: 'Remove Car', 
        supplierReportTitle: 'Country-Based Supplier Volume Report', supplierReportSub: 'Suppliers grouped by country', 
        metaTitle: 'Meta-Search Integration Center', metaSub: 'Open API address for Skyscanner and Kayak.', 
        feedAddress: 'Official JSON Feed Address', copy: 'Copy', footer: 'All Rights Reserved © 2026 FlexiDrive Global OS.',
        ...
        dailyNetEarn: 'Daily Net Earning (Max 400 €)', saveAndPublish: 'Save Car to System',
        modalLocation: 'Country / Location:', modalSupplier: 'Supplier Company:', modalContact: 'Contact (Phone):', mapBtn: 'Show Location on Map (Google Maps)',
        taxBadge: 'Tax & Financial File', fullNameLabel: 'Authorized Full Name:', taxNoLabel: 'Tax Identification Number (TIN):', taxOfficeLabel: 'Tax Office:', taxAddressLabel: 'Registered Address:', taxStatusText: 'Tax Compliance Status: Active & Compliant'
      },
      de: { 
        adminLoginTitle: 'FlexiDrive Admin HQ', adminLoginSub: 'Geben Sie Ihr Administrator-Passwort ein.', adminLoginBtn: 'Sicherer Login',
        quickMenu: 'Schnellmenü', fleet: 'Flottenbetrieb', suppliers: 'Lieferantennetzwerk', feed: 'Meta-Search Feed', supplierPortal: 'Lieferantenportal', 
        supplierBadge: 'Lieferant', totalFleet: 'Gesamte Flotte', activeAvailable: 'Aktiv / Verfügbar', dailyRevenue: 'Täglicher Umsatz', 
        available: 'VERFÜGBAR', rented: 'VERMIETET', netSale: 'Netto / Verkauf:', published: 'Veröffentlicht:', changeStatus: 'Status Ändern', removeCar: 'Entfernen', 
        supplierReportTitle: 'Länderbasierter Lieferantenbericht', supplierReportSub: 'Lieferanten nach Ländern gruppiert', 
        metaTitle: 'Meta-Search Integrationszentrum', metaSub: 'Offene API-Adresse für Skyscanner und Kayak.', 
        feedAddress: 'Offizielle JSON Feed Adresse', copy: 'Kopieren', footer: 'Alle Rechte vorbehalten © 2026 FlexiDrive Global OS.',
        ...
        dailyNetEarn: 'Ttäglicher Nettoverdienst (Max 400 €)', saveAndPublish: 'Fahrzeug Speichern',
        modalLocation: 'Land / Standort:', modalSupplier: 'Lieferant:', modalContact: 'Kontakt (Telefon):', mapBtn: 'Standort auf Karte anzeigen (Google Maps)',
        taxBadge: 'Steuer- und Finanzakte', fullNameLabel: 'Vollständiger Name:', taxNoLabel: 'Steuernummer:', taxOfficeLabel: 'Finanzamt:', taxAddressLabel: 'Registrierte Adresse:', taxStatusText: 'Steuerstatus: Aktiv & Konform'
      },
      it: { 
        adminLoginTitle: 'FlexiDrive Admin HQ', adminLoginSub: 'Inserisci la password di amministrazione.', adminLoginBtn: 'Accesso Sicuro',
        quickMenu: 'Menu Rapido', fleet: 'Operazioni Flotta', suppliers: 'Rete Fornitori', feed: 'Meta-Search Feed', supplierPortal: 'Portale Fornitori', 
        supplierBadge: 'Fornitore', totalFleet: 'Flotta Totale', activeAvailable: 'Attivo / Disponibile', dailyRevenue: 'Potenziale Ricavo', 
        available: 'DISPONIBILE', rented: 'AFFITTATO', netSale: 'Netto / Vendita:', published: 'Pubblicato:', changeStatus: 'Cambia Stato', removeCar: 'Rimuovi', 
        supplierReportTitle: 'Rapporto Fornitori per Paese', supplierReportSub: 'Raggruppati per paese', 
        metaTitle: 'Centro Integrazione Meta-Search', metaSub: 'Indirizzo API aperto per Skyscanner e Kayak.', 
        feedAddress: 'Indirizzo JSON Feed Ufficiale', copy: 'Copia', footer: 'Tutti i diritti riservati © 2026 FlexiDrive Global OS.',
        ...
        dailyNetEarn: 'Guadagno Netto Giornaliero (Max 400 €)', saveAndPublish: 'Salva Veicolo',
        modalLocation: 'Paese / Posizione:', modalSupplier: 'Fornitore:', modalContact: 'Contatto (Telefono):', mapBtn: 'Mostra posizione sulla mappa (Google Maps)',
        taxBadge: 'Fascicolo Fiscale e Finanziario', fullNameLabel: 'Nome e Cognome:', taxNoLabel: 'Partita IVA / Codice Fiscale:', taxOfficeLabel: 'Agenzia delle Entrate:', taxAddressLabel: 'Indirizzo Registrato:', taxStatusText: 'Stato Fiscale: Attivo e Conforme'
      }
    };
      ...
      Alpine.data('adminApp', () => ({
        ...
        openTaxDetails(supplier) {
          ...
          this.selectedTaxSupplier = {
            ...supplier,
            fullName: found ? (found.fullName || 'Eren Evren Barış') : 'Eren Evren Barış',
            email: found ? (found.email || 'eren@coca-cola.com') : 'eren@coca-cola.com',
...
// 5. TEDARİKÇİ PORTALI (Red & Gold Edition + Bütün Diller Tam Entegre)
...
app.listen(PORT, () => {
  console.log(`FlexiDrive Red & Gold Edition (Tam Entegre) sunucusu http://localhost:${PORT} adresinde aktif!`);
});
