...
// 4. ANA YÖNETİCİ PANELİ (Admin HQ)
app.get('/', (req, res) => {
  ...
  <header class="bg-slate-900 border-b border-slate-800 sticky top-0 z-40">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      <div class="flex items-center space-x-3">
        <div class="bg-indigo-600 text-white p-2 rounded-xl flex items-center justify-center font-black text-lg shadow-lg shadow-indigo-500/30"><i class="fa-solid fa-shield-halved"></i></div>
        <div>
          ...
          <span class="text-[10px] text-indigo-400 ml-1 font-bold bg-indigo-500/10 px-2.5 py-0.5 rounded-full border border-indigo-500/20">Admin HQ</span>
        </div>
      </div>
      <div class="flex items-center space-x-2">
        <button @click="activeTab = 'admin'" :class="activeTab === 'admin' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'" class="px-4 py-2 rounded-xl font-semibold text-xs transition-all">
          <i class="fa-solid fa-car mr-1.5"></i> Filo Operasyonları
        </button>
        <button @click="activeTab = 'partners'" :class="activeTab === 'partners' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'" class="px-4 py-2 rounded-xl font-semibold text-xs transition-all">
          <i class="fa-solid fa-earth-europe mr-1.5"></i> Tedarikçi Ağı (Ülke Bazlı)
        </button>
        <button @click="activeTab = 'integrations'" :class="activeTab === 'integrations' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'" class="px-4 py-2 rounded-xl font-semibold text-xs transition-all">
  ...
  <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div x-show="activeTab === 'admin'" x-transition>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm flex justify-between items-center"><div><p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Komisyon</p><h3 class="text-3xl font-black mt-1 text-cyan-400">%20</h3></div><div class="text-cyan-400 text-3xl opacity-50"><i class="fa-solid fa-percent"></i></div></div>
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between">
            <div>
              ...
              <div class="bg-slate-950 p-2.5 rounded-xl my-3 text-xs space-y-1 border border-slate-800">
                ...
                <div class="flex justify-between"><span class="text-slate-500">Yayınlanma:</span><span class="font-bold text-indigo-300" x-text="new Date(car.createdAt).toLocaleString('tr-TR')"></span></div>
              </div>
            </div>
            ...
            <div class="pt-3 border-t border-slate-800 flex justify-between items-center text-xs">
              ...
              <button @click="deleteCar(car._id)" class="bg-rose-500/10 hover:bg-rose-600 text-rose-400 hover:text-white border border-rose-500/20 px-3 py-1.5 rounded-lg font-bold transition-all"><i class="fa-solid fa-trash-can mr-1"></i> Aracı Kaldır</button>
            </div>
          </div>
        </template>
      </div>
              ...
              <div class="flex justify-between items-center mb-6 pb-4 border-b border-slate-800">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-slate-400">Toplam <strong class="text-indigo-400" x-text="group.suppliers.length"></strong> Firma / <strong class="text-emerald-400" x-text="group.totalCars"></strong> Araç Hacmi</p>
                  </div>
                </div>
              </div>
              <div class="space-y-3">
                <template x-for="supplier in group.suppliers" :key="supplier.name">
                  <div class="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex justify-between items-center">
                    <div>
                      <h4 class="text-sm font-bold text-white" x-text="supplier.name"></h4>
                      <p class="text-xs text-slate-400 mt-1"><i class="fa-solid fa-phone text-emerald-400 mr-1"></i> <span class="font-mono" x-text="supplier.contact"></span></p>
                    </div>
                    <span class="bg-indigo-500/10 text-indigo-300 text-xs font-bold px-3 py-1 rounded-full" x-text="supplier.carCount + ' Araç'"></span>
                  </div>
                </template>
              </div>
            </div>
    ...
    <div x-show="activeTab === 'integrations'" x-cloak x-transition>
      <div class="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        <div class="flex items-center space-x-3 mb-6 border-b border-slate-800 pb-4">
          <div class="bg-indigo-600 text-white p-3 rounded-xl flex items-center justify-center text-xl shadow-lg"><i class="fa-solid fa-satellite-dish"></i></div>
          <div>
            <h2 class="text-xl font-black text-white">Meta-Search Entegrasyon Merkezi</h2>
            <p class="text-xs text-slate-400">Skyscanner ve Kayak gibi platformların envanterinizi çekeceği açık API adresi.</p>
          </div>
        </div>

        <div class="space-y-4">
          <div class="bg-slate-950 p-6 rounded-2xl border border-slate-800 space-y-2">
            <span class="text-xs font-bold text-indigo-400 uppercase tracking-wider block">Resmi JSON Feed Bağlantı Adresi</span>
            <div class="flex space-x-2">
              <input type="text" readonly :value="windowOrigin + '/api/feed/global-inventory'" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-emerald-400 font-mono focus:outline-none">
              <button @click="navigator.clipboard.writeText(windowOrigin + '/api/feed/global-inventory'); alert('URL kopyalandı!')" class="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-4 py-2.5 rounded-xl text-xs whitespace-nowrap shadow">Kopyala</button>
            </div>
          </div>
        </div>
      </div>
...
// 5. TEDARİKÇİ PORTALI (Orijinal Sade & Şık Sürüm)
...
<body class="h-full text-slate-100 flex flex-col" x-data="supplierPortal()">
  ...
  <header class="bg-slate-900 border-b border-slate-800 sticky top-0 z-40">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      <div @click="activeTab = 'cars'" class="flex items-center space-x-3 cursor-pointer group" title="Ana Menüye Dön">
        <div class="bg-emerald-600 text-white p-2.5 rounded-xl flex items-center justify-center font-black text-lg shadow-lg shadow-emerald-500/30 group-hover:scale-105 transition-transform"><i class="fa-solid fa-car-side"></i></div>
        <div>
          <span class="text-xl font-extrabold tracking-tight text-white group-hover:text-emerald-400 transition-colors">FlexiDrive</span> 
      ...
      <div class="flex items-center space-x-2" x-show="isLoggedIn">
        <button @click="activeTab = 'cars'" :class="activeTab === 'cars' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'" class="px-4 py-2 rounded-xl font-semibold text-xs transition-all flex items-center"><i class="fa-solid fa-car mr-1.5"></i> Araçlarım</button>
        <button @click="activeTab = 'wallet'" :class="activeTab === 'wallet' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'" class="px-4 py-2 rounded-xl font-semibold text-xs transition-all flex items-center"><i class="fa-solid fa-wallet mr-1.5"></i> Hesap Özeti</button>
        <button @click="activeTab = 'stats'" :class="activeTab === 'stats' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'" class="px-4 py-2 rounded-xl font-semibold text-xs transition-all flex items-center"><i class="fa-solid fa-chart-line mr-1.5"></i> İstatistikler</button>
        <button @click="activeTab = 'add'" :class="activeTab === 'add' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'" class="px-4 py-2 rounded-xl font-semibold text-xs transition-all flex items-center border border-emerald-500/30"><i class="fa-solid fa-plus-circle mr-1.5"></i> Yeni Araç Ekle</button>
        <button @click="logout()" class="text-rose-400 hover:bg-rose-500/10 p-2 rounded-xl text-xs transition-all ml-2" title="Çıkış Yap"><i class="fa-solid fa-right-from-bracket text-base"></i></button>
      </div>
    </div>
  </header>
  ...
  <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col justify-center">
    ...
    <div x-show="!isLoggedIn" class="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-3xl p-8 shadow-2xl text-center">
      <div class="w-16 h-16 bg-emerald-600/20 text-emerald-400 rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4 border border-emerald-500/30"><i class="fa-solid fa-building-user"></i></div>
      ...
      <form @submit.prevent="loginSupplier()" class="space-y-4">
        ...
        <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-3 rounded-xl shadow-lg shadow-emerald-600/20 transition-all text-sm">Panele Giriş Yap <i class="fa-solid fa-arrow-right ml-2"></i></button>
      </form>
    </div>

    <div x-show="isLoggedIn" x-cloak class="w-full space-y-6">
      
      <div @click="activeTab = 'cars'" class="bg-gradient-to-r from-slate-900 via-emerald-950/20 to-slate-900 border border-emerald-500/20 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-center cursor-pointer hover:border-emerald-500/40 transition-all">
        <div class="flex items-center space-x-4 mb-4 md:mb-0">
          ...
          <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5"><i class="fa-solid fa-circle-check text-emerald-400 mr-1"></i> Aktif Tedarikçi Paneli (Ana Menüye Dön)</p>
          </div>
        </div>
        <div class="flex space-x-4 bg-slate-950 p-3 rounded-2xl border border-slate-800 text-xs text-center">
      ...
      <div x-show="activeTab === 'cars'" x-transition>
        <div class="flex justify-between items-center mb-6">
          ...
          <button @click="activeTab = 'add'" class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-4 py-2 rounded-xl text-xs transition-all shadow"><i class="fa-solid fa-plus mr-1"></i> Yeni Araç Ekle</button>
        </div>
      ...
      <div x-show="activeTab === 'wallet'" x-cloak x-transition>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            ...
            <div class="text-xl font-bold text-cyan-400">Global B2B Dağıtım Sözleşmesi</div>
      ...
      <div x-show="activeTab === 'add'" x-cloak x-transition class="bg-slate-900 border border-slate-700 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <p class="text-xs text-slate-400 mb-6">Firma adınız otomatik eşleştirilmektedir: <strong class="text-emerald-400" x-text="companyName"></strong> (Maksimum günlük net kazanç sınırı 400 €'dur).</p>
        ...
        <form @submit.prevent="submitCar" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
              ...
              <div class="flex space-x-3">
                ...
                <div class="w-2/3">
                  <label class="block text-[10px] font-black text-emerald-400 uppercase tracking-wider mb-1">Günlük Net Kazanç (Max 400 €)</label>
                  <div class="relative">
          ...
          <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-xl shadow-lg transition-all"><i class="fa-solid fa-cloud-arrow-up mr-2"></i> Aracı Sisteme Kaydet ve Listeme Ekle</button>
        </form>
      </div>
...
app.listen(PORT, () => {
  console.log(`FlexiDrive sunucusu http://localhost:${PORT} adresinde aktif!`);
});
